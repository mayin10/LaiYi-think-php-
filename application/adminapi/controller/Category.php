...
class Category extends BaseApi
{
    public function index()
    {
        ...
        $list = \app\common\model\Category::where($where)->order(['sort'=>'asc'])->paginate($listRow);
        foreach ($list as $v){
            $v['wls_img'] = $this->get_wls_img('category', $v->id);
        }
        unset($v);
        $this->ok($list);
    }
    ...
    public function datalist(){
        $list = \app\common\model\Category::order(['sort'=>'asc'])->select();
        $this->ok($list);
    }
    ...
    public function save(Request $request)
    {
        ...
        $this->addImg($params['wls_img'], $info->id,'category',530,180);
       // $cate= \app\common\model\Category::find($info['id']);
        $this->ok($params);
    }
    ...
    public function read($id)
    {
        ...
        $data['wls_img_in'] = $this->getImgList('category',$id);
        $data['wls_img'] = '';
        $data['wls_img_remove'] = [];
        $this->ok($data);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->addImg($params['wls_img'], $id,'category',530,180);
    ...
    public function hotChange(){
        $params = input();
        \app\common\model\Category::update(['is_hot'=>$params['is_hot']], ['id' => $params['id']], true);
        $this->ok($params);
    }
    ...
    public function delete($id)
    {
        ...
        $this->removeImgs(null,  $id,'category');
