...
class Auth extends BaseApi
{
    ...
    public function index()
    {
        ...
        if(!empty($params['keyword'])){
            $where['auth_name|auth_c|auth_a'] = ['like', "%{$params['keyword']}%"];
        }
        $where['level'] = 0;
        $listRow = 6;
        ...
        $list = \app\common\model\Auth::field('id,auth_name,pid,sort,auth_c,auth_a,is_nav,level')
            ->where($where)->order('sort asc')->paginate($listRow);

        foreach($list as $v){
            $v['is_nav'] = $v['is_nav'] == 1? true:false;
            $list1= \app\common\model\Auth::field('id,auth_name,pid,sort,auth_c,auth_a,is_nav,level')
                ->where(['pid'=>$v->id])->order('sort asc')->select();

            foreach ($list1 as $v1){
                $list2= \app\common\model\Auth::field('id,auth_name,pid,sort,auth_c,auth_a,is_nav,level')
                    ->where(['pid'=>$v1->id])->order('sort asc')->select();
                $v1['children'] = $list2;
            }
            $v['children'] = $list1;
        }
